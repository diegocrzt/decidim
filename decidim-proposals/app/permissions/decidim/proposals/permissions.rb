  ...
  module Proposals
    class Permissions
      ...
      def can_withdraw_proposal?
        permission_action.action == :withdraw &&
          proposal &&
          proposal.author == user
      end

      def can_endorse_proposal?
        permission_action.action == :endorse &&
          proposal &&
          authorized?(:endorse) &&
      ...
      def can_unendorse_proposal?
        permission_action.action == :unendorse &&
          proposal &&
          authorized?(:endorse) &&
          current_settings&.endorsements_enabled?
      end
      ...
      def can_unvote_proposal?
        permission_action.action == :unvote &&
          proposal &&
          authorized?(:vote) &&
          voting_enabled?
      end
